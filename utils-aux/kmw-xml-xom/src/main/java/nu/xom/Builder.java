...
public class Builder {
    ...
    static XMLReader findParser(boolean validate) {
        ...
        catch (SAXException | NoClassDefFoundError ex) {
            // look for next one
        }
        ...
        catch (SAXException | InstantiationException | ClassNotFoundException | IllegalAccessException | NoClassDefFoundError ex) {
            // look for next one
        }
        // look for next one
        // look for next one
    ...
    private String canonicalizeURL(String uri) {
        ...
        try {
            ...
            if (port >= 0) canonicalForm.append(":").append(port);
            canonicalForm.append(path);
            if (query != null) canonicalForm.append("?").append(query);
    ...
    private Document build(InputSource in) 
      throws ParsingException, ValidityException, IOException {
        ...
        catch (RuntimeException | UTFDataFormatException | CharConversionException ex) {
